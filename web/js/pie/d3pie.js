...
// taken from jQuery
    ...
    var segments = {
        ...
        create: function (pie) {
            ...
            //begin lianglei 3.30 add
            if (pie.options.data.content[pie.options.data.content.length - 1].isGrouped == true) {
                var totalOtherScore = 0;
                var num = 0;
                var len = pie.options.data.content.length;
                var groupedData = pie.options.data.content[len - 1].groupedData;
                for (var i = 0; i < groupedData.length; i++) {
                    totalOtherScore += (groupedData[i].score * groupedData[i].value);
                    num += groupedData[i].value;
                }
                avgOtherScore = totalOtherScore / num;
            }
            //end lianglei 3.30 add
            var arc = d3.svg.arc()
                .innerRadius(pie.innerRadius)
                //.outerRadius(pie.outerRadius)
                .outerRadius(function (d) {
                    //lianglei 3.30 modefied
                    if (d.score) {
                        return pie.outerRadius * d.score / 5;
                    }
                    else {
                        return pie.outerRadius * avgOtherScore / 5;
                    }
                })
                .startAngle(0)
                .endAngle(function (d) {
