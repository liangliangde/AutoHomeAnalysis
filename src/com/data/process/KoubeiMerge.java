...
/**
 * Created by llei on 16-5-30.
 */
public class KoubeiMerge {
    public static void main(String args[]) throws IOException {
        Set<String> existKoubeiSet = VariousMap.koubeiSet();
        File dist = new File("/home/llei/IdeaProjects/autohome/koubei_yage");
        File[] files = dist.listFiles();
        for(File file : files){
            System.out.println(file.getName());
            if(file.getName().equals("user_id"))
                continue;
            InputStreamReader read = new InputStreamReader(new FileInputStream(file), "UTF-8");
            BufferedReader bufferedReader = new BufferedReader(read);
            String lineTxt = null;
            while ((lineTxt = bufferedReader.readLine()) != null) {
                getJSONContent(lineTxt, existKoubeiSet);
            }
        }
        System.out.print("finish");
    }

    private static void getJSONContent(String JSONText, Set<String> existKoubeiSet) throws IOException {
        ...
        try {
            ...
            if(existKoubeiSet.contains(kid)){
                return; //该口碑已有时返回；
            }
//            System.out.println(cid + ", " + kid);
            String styleid = JSONObject.getString("sid").trim();
            String uid = JSONObject.getString("uid").trim();

            String score = JSONObject.getString("car_info").trim();
            JSONTokener scoreTokener = new JSONTokener(score);
            JSONObject scoreObject;
            scoreObject = (JSONObject) scoreTokener.nextValue();
            String interior = scoreObject.getString("内饰").trim();
            String power = scoreObject.getString("动力").trim();
            String appearence = scoreObject.getString("外观").trim();
            String costPerformance = scoreObject.getString("性价比").trim();
            String control = scoreObject.getString("操控").trim();
            String oil = scoreObject.getString("油耗").trim();
            String space = scoreObject.getString("空间").trim();
            String comfort = scoreObject.getString("舒适性").trim();
            String price = scoreObject.getString("裸车购买价").replace('\n', ' ').trim();
            String boughtSite = scoreObject.getString("购买地点").replace('\n', ' ').replace(' ',' ').trim();
            String boughtTime = scoreObject.getString("购买时间").replace('\n', ' ').trim();
            String boughtSeries = scoreObject.getString("购买车型").replace('\n', ' ').trim();
            String aim = scoreObject.getString("购车目的").replace('\n', ' ').trim();
            ...
            String first = (String)contentObject.keys().next();
            System.out.println(first);
            String koubei = contentObject.getString(first).trim();
